...
app.post('/api/agents/:agentId/run', async (req, res) => {
  ...
  try {
    console.log(`Manually triggering agent: ${agentToRun.name} (ID: ${agentToRun.id})`);
    // Await the agent's execution to capture the outcome.
    await agentToRun.run();
    res.status(200).json({ message: `Agent '${agentToRun.name}' ran successfully.` });
  } catch (error) {
